    ...
    <div className="px-4 pt-6">
      ...
      {isLoading ? (
        ...
        <EmptyState
          icon="💛"
          title="No likes yet"
          description="Keep your profile updated to get more likes."
        />
      ) : (
        <div className="grid grid-cols-2 gap-3">
          {likes.map((l: any) => {
            ...
            return (
              <div
                key={l.id}
                className="flex flex-col items-center gap-2 rounded-2xl border border-white/5 bg-smoke p-4 text-center"
              >
                <Avatar
                  src={user.avatar_url}
                  name={user.full_name}
                ...
                {user.price_min && (
                  <span className="text-xs text-ember">
                    {formatNaira(user.price_min)}+
                  </span>
                )}
                {l.action === 'superlike' && (
                  <span className="text-xs text-ember">⭐ Superliked you</span>
                )}
              </div>
            )
          })}
