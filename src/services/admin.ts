...
export async function getAdminStats() {
  const [users, chefs, matches, bookings, disputes, pendingKyc] = await Promise.all([
    sb().from("profiles").select("id", { count: "exact", head: true }),
    sb().from("profiles").select("id", { count: "exact", head: true }).eq("role", "cook"),
...
export async function suspendUser(userId: string, suspended: boolean) {
  await sb().from("profiles").update({ suspended } as any).eq("id", userId);
}
